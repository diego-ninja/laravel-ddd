...
namespace Modules\Sales\Application\Middleware\Events;
...
class EventStoreMiddleware
{
    ...
    private function checkForDuplicates(DomainEvent $event): void
    {
        ...
        if ($exists) {
            throw SalesDomainException::businessRuleViolation(
    ...
    private function validateEvent(DomainEvent $event): void
    {
        ...
        if (empty($event->eventId)) {
            throw SalesDomainException::invalidData('Event ID cannot be empty');
        }

        if (empty($event->aggregateId)) {
            throw SalesDomainException::invalidData('Aggregate ID cannot be empty');
        }
    ...
    private function validateEventSpecificData(DomainEvent $event): void
    {
        ...
        //         throw SalesDomainException::invalidData('Customer ID required for OrderWasPlaced');
        //     }
        // }
    }
