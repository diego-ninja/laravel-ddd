...
use Modules\Sales\Application\DTOs\CustomerSummary;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Cache;

class FindActiveCustomersQueryHandler
{
    ...
    /**
     * Handle the FindActiveCustomers query.
     * 
     * @param array $criteria Query criteria/filters
     * @return CustomerSummary[]|CustomerSummary|null
     */
     ...
     */
    public function handleSingle(array $criteria = []): ?CustomerSummary
    {
    ...
    {
        $query = DB::table('customerSummary_read_models');
    ...
    private function mapToReadModels(\Illuminate\Support\Collection $results): array
    {
        return $results->map(function ($row) {
            return CustomerSummary::fromArray((array) $row);
        })->toArray();
    }
    ...
    {
        return 'customerSummary_query_' . md5(serialize($criteria));
    }
}